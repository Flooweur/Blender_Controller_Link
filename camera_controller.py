    ...
    def _update_look_at_camera(self, inputs: dict):
        ...
        if 'rightstick_x' in inputs:
            ...
            if direction.length > 0.001:  # Avoid division by zero
                ...
                # Check if cross product resulted in zero vector (camera directly above/below target)
                if perpendicular.length > 0.001:
                    perpendicular.normalize()
                    delta_location += perpendicular * inputs['rightstick_x'] * self.sensitivity * 0.1
                else:
                    # Use alternative perpendicular vector when camera is above/below target
