...
class CLE_OT_AddCustomInput(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        # Show available inputs as buttons for quick selection
        cle_reader = bpy.data.objects.get("CLE_reader")
        if cle_reader:
            available_inputs = self._get_available_inputs(context)
            if available_inputs:
                layout.label(text="Available Inputs:")
                box = layout.box()
                # Show inputs in a grid layout
                for i, (prop_name, display_name) in enumerate(available_inputs):
                    if i % 3 == 0:  # Start new row every 3 buttons
                        row = box.row()
                    op = row.operator("wm.cle_set_mapped_input", text=display_name)
                    op.input_name = prop_name
    ...
    def invoke(self, context, event):
        ...
        # No need to update enum dynamically - we'll use a different approach
        
        return context.window_manager.invoke_props_dialog(self, width=400)
    
    def _get_available_inputs(self, context):
        """Get list of available controller inputs."""
        cle_reader = bpy.data.objects.get("CLE_reader")
        if cle_reader:
            available_inputs = []
...
class CLE_OT_SetMappedInput(bpy.types.Operator):
    """
    Operator to set the mapped input from the enum selection.
    """
    bl_idname = "wm.cle_set_mapped_input"
    bl_label = "Set Mapped Input"
    bl_description = "Set the mapped input field from the selected controller input"
    ...
    def execute(self, context):
        """Set the mapped input field."""
        try:
            if hasattr(context.scene, '_temp_custom_input'):
                temp_input = context.scene._temp_custom_input
                temp_input.mapped_input = self.input_name
                self.report({'INFO'}, f"Set mapped input to: {self.input_name}")
                return {'FINISHED'}
            else:
                self.report({'ERROR'}, "No temporary input found")
                return {'CANCELLED'}
        except Exception as e:
            logger.error(f"Error setting mapped input: {e}")
            self.report({'ERROR'}, f"Failed to set mapped input: {e}")
            return {'CANCELLED'}
...
def register_custom_input_classes():
    ...
    bpy.utils.register_class(CLE_OT_SetMappedInput)
...
def unregister_custom_input_classes():
    ...
    bpy.utils.unregister_class(CLE_OT_SetMappedInput)
